// Client-side log redaction utility
// Mirrors backend redactForLogs.js for frontend use
...
const isTextKey = (key) => {
  const k = String(key || '');
  return /(text|message|note|prompt|query|q|content|diagnosis|symptom|complaint|assessment|plan|history|transcript|description|notes)/i.test(k);
};
