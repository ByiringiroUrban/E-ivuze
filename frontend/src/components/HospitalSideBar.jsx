...
import {
    FaChartLine, FaUserMd, FaUsers, FaExchangeAlt,
    FaClipboardList, FaCog, FaSignOutAlt
} from 'react-icons/fa';

const HospitalSideBar = () => {
    const navigate = useNavigate();
    const { hToken, logoutHospital } = useContext(HospitalContext);
    ...
    const menuItems = [
        { to: '/hospital-dashboard/', icon: <FaChartLine />, label: t('hospital.sidebar.dashboard') || 'Dashboard', end: true },
        { to: '/hospital-dashboard/doctors', icon: <FaUserMd />, label: t('hospital.sidebar.doctors') || 'Doctors' },
        { to: '/hospital-dashboard/patients', icon: <FaUsers />, label: t('hospital.sidebar.patients') || 'Patients' },
        { to: '/hospital-dashboard/transfers', icon: <FaExchangeAlt />, label: t('hospital.sidebar.transfers') || 'Transfers' },
        { to: '/hospital-dashboard/reports', icon: <FaClipboardList />, label: t('hospital.sidebar.reports') || 'Reports' },
        { to: '/hospital-dashboard/settings', icon: <FaCog />, label: t('nav.settings') || 'Settings' },
    ];
            ...
            <div className={`flex items-center h-16 px-4 border-b border-[#ffffff10] ${!isOpen && 'justify-center'}`}>
                {isOpen ? (
                    <div className="flex items-center gap-3">
                        ...
                        <span className="font-bold text-lg tracking-tight uppercase whitespace-nowrap overflow-hidden">HOSPITAL</span>
                    </div>
                ) : (
                    <img src={assets.logo} alt="Logo" className="w-8 h-8 object-contain" />
                )}
